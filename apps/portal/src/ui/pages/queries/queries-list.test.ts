...
describe('queries-list', () => {
  ...
  it('renders query rows with edit links', () => {
    el.setQueries(queries(2))
    const links = el.querySelectorAll<HTMLAnchorElement>('.query-row-text')
    expect(links.length).toBe(2)
    expect(links[0]?.textContent).toContain('Staff Engineer 1')
    expect(links[0]?.textContent).toContain('(Sydney)')
    expect(links[0]?.getAttribute('href')).toBe('#queries/edit?id=1')
  })
  ...
  it('dispatches queries-list:toggle from the enabled checkbox', () => {
