...
export function toBody(record: Record<string, unknown>): string {
  const { level: _level, time, scope: _scope, ...rest } = record
...
export default async function (opts: { dbPath: string }) {
  ...
  const insert = db.prepare('INSERT INTO logs (level, scope, body) VALUES (?, ?, ?)')
  return build(
    async function (source: AsyncIterable<Record<string, unknown>>) {
      for await (const record of source) {
        insert.run(Number(record.level), (record.scope as string | null) ?? null, toBody(record))
      }
    },
    {
      close: async () => {
